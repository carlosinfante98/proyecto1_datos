...
public class OrdenatorP<T extends Comparable<T>> 
{
	...
	public void ordenar(Ordenamientos algo, boolean ascendente, ComparatorT<T> comparador, ArrayListT<T> lista)
	{
		switch(algo)
		{
		case MERGE:
	...
	private void quickSort(ArrayListT<T> lista, boolean ascendente, ComparatorT<T> comparador)
	{
		shuffleArray(lista);
		quickSorting(lista, 0, lista.size()-1, ascendente, comparador);
	}

	private void quickSorting(ArrayListT<T> lista, int low, int high, boolean ascendente, ComparatorT<T> comparador)
	{
		if (low < high)
		{
	...
	private void shellSort(ArrayListT<T> lista, boolean ascendente, ComparatorT<T> comparador)
	{
		int inner, outer;
		T temp;
	 ...
	 */
	private void insertionSort(ArrayListT<T> lista, boolean ascendente, ComparatorT<T> comparador)
	{
    ...
    of pivot */
	private int quickSortpartition(ArrayListT<T> lista, int low, int high, boolean ascendente, ComparatorT<T> comparador)
	{
	...
	private void mergeSort(ArrayListT<T> a, boolean ascendente, ComparatorT<T> comparador)
	{
		ArrayListT<T> aux = new ArrayListT<T>(a.size());
		sort(a, aux, 0, a.size() - 1, ascendente, comparador);
	}

	private void sort(ArrayListT<T> a, ArrayListT<T> aux, int lo, int hi, boolean ascendente, ComparatorT<T> comparador)
	{
		if (hi <= lo) return;
		int mid = lo + (hi - lo) / 2;
	...
	private void merge(ArrayListT<T> a, ArrayListT<T> aux, int lo, int mid, int hi, boolean ascendente, ComparatorT<T> comparador)
	{
	...
	public void shuffleArray(ArrayListT<T> a)
	{
		int n = a.size();
	...
	private void swap(ArrayListT<T> a, int i, int change)
	{
		T helper = a.get(i);
